...
Proof.
  ...
  { generalize valid_wd valid_wd0. clear. intros.
    ...
    eapply valid_wd0 in H; eauto. eapply valid_wd in H; eauto. lia.
    destruct H. auto.
    eapply valid_wd in H; eauto. eapply valid_wd0 in H; eauto. lia.
  }
  ...
  all: try(Esimpl;eauto;econstructor;eauto;apply MemPostemp).
  {
    ...
    1-4:inv MEMPRE;
      ...
      unfold load_fp in *;simpl in *;
        erewrite H;eauto;
          try (unfold belongsto,Locs.belongsto,range_locset;ex_match2;try destruct zle,zlt;try lia;auto);
          try (unfold Mem.range_perm,Mem.perm in r0;unfold GMem.perm,strip;simpl;apply r0;lia).
    ...
    unfold load_fp in *;simpl in *.
    unfold belongsto,Locs.belongsto,range_locset;ex_match2;try destruct zle,zlt;try lia;auto.
    unfold Mem.range_perm,Mem.perm in r0;unfold GMem.perm,strip;simpl;apply r0;lia.
