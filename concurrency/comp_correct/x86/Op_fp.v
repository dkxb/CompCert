...
Proof.
  ...
  subst. exists b, ofs, delta. split;[auto|left;split;[lia|]].
...
Proof.
  ...
  - constructor; try apply loc_inject_emp.
    ...
    assert (ofs0 = Ptrofs.unsigned (Ptrofs.add ofs (Ptrofs.repr delta)) \/
            ofs0 = Ptrofs.unsigned (Ptrofs.add ofs (Ptrofs.repr delta)) - 1) as HOFS by lia.
    destruct HOFS as [HOFS|HOFS];[left|right];(split; [lia|]).
    destruct eq_block; try contradiction.
    destruct zle, zlt; try lia; auto.
    destruct eq_block; try contradiction.
    destruct zle, zlt; try lia; auto.
