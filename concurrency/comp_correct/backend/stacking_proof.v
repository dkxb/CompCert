...
Local Opaque b fe.
  ...
  clear; intros. destruct LAYOUT1. subst b fe. lia. 
...
Proof.
  clear. induction l as[|a h']; simpl; [| pose proof (typesize_pos a)]; lia.
Qed.
...
Proof.
  ...
  -
    assert (n<=length tyl0)%nat by lia. specialize (IHn H5); clear H5.
    assert (Datatypes.S (length tyl0 - Datatypes.S n) = length tyl0 - n)%nat by lia.
    ...
    { clear. intros.
      ...
      { clear Heqh Heqt. generalize dependent n. clear. intros n H C.
        ...
        rewrite H0 in H1. assert (length (skipn n l) > 0)%nat by lia.
        rewrite C in H2. simpl in H2. lia. }
      destruct (skipn n l) eqn:?; [contradiction|].
      exists a. simpl. auto. rewrite Heqh. apply firstn_length_le; lia. }
    exploit (H6 _ tyl0 headN). subst headN; lia. intros [ty Htyl0].
    exploit (H6 _ args0 headN). subst headN; lia. intros [v Hargs0].
    ...
    { rewrite Htyl0. subst ofsn headn. generalize (firstn headN tyl0) (skipn (headN+1) tyl0).
      clear. intros l l0. induction l; intros; simpl; pose proof (tyl_length_non_neg l0); lia. }
    ...
    { generalize H3 H0 Htyl0 Hargs0; clear. generalize headN. clear. intros n H.
      ...
      { destruct (le_dec n (length args0)).
        ...
        rewrite firstn_length_le; try congruence.
        rewrite firstn_all2; try lia. rewrite firstn_all2; try lia. }
    ...
    { unfold loc_arguments. destruct Archi.ptr64 eqn:C; inversion C; clear C. rewrite H1.
      rewrite Htyl0. generalize H4 H5 Htyl0 Heqofsn. clear. intros. assert (headN < length tyl0)%nat by (subst headN; lia).
      ...
      intros. destruct n0.
      simpl in *. subst; auto.
      simpl in *. apply IHtyl0; clear IHtyl0; [lia|congruence|lia]. }
    ...
    { generalize Hargs_v' Hargs0 H3. clear. intros. subst headN. rewrite Hargs0, <-H3 in Hargs_v'. clear Hargs0 H3.
      assert (length args0 - (Datatypes.S n) <= length args0)%nat by lia.
    ...
    (* Tlong *)    
    { subst ty. simpl. 
      exploit (H8 (loadframe.tyl_length headn) Tint); try (simpl in *; lia; fail). apply Z.divide_1_l. 
      rewrite <- Heqofsn. intros (vlo & LOAD2 & INJ2).
      exploit (H8 (loadframe.tyl_length headn + 1) Tint); try (simpl in *; lia; fail). apply Z.divide_1_l.
      ...
      { match goal with |- context[loadframe.tyl_bytes ?x] => pose proof (loadframe.tyl_bytes_non_neg x); lia end. }
      ...
      assert (length tyl0 - Datatypes.S n < length tyl0)%nat by lia.
      generalize (length tyl0 - Datatypes.S n)%nat Htyl0 H15. clear.
      { intro n. generalize (skipn (n + 1) tyl0)%nat. intros l Htyl0 H0.
        assert (length (firstn n tyl0) = n) by (apply firstn_length_le; lia).
      ...
      pose proof (loadframe.tyl_bytes_non_neg (skipn (headN + 1) tyl0)). lia.
    }
    
    (* not Tlong *)
    {
      exploit H8; eauto; try lia; try (destruct ty; try contradiction; apply Z.divide_1_l).
      ...
      { subst ofsn. rewrite loadframe.tyl_length_agree. 
        ...
        pose proof (firstn_length_le tyl0). specialize (H (length tyl0 - n)%nat). exploit H; try lia.
        clear. generalize (length tyl0 - n)%nat, (firstn (length tyl0 - n) tyl0). clear.
        intros. rewrite <- Nat.add_1_r, <- H, firstn_app_2, loadframe.tyl_bytes_app. simpl. lia. }
      assert (0 <= ofsn * 4 <= Ptrofs.max_unsigned).
      { subst ofsn headn. split; try lia. generalize H10 H. clear. intros.
        pose proof (typesize_pos ty). unfold Ptrofs.max_unsigned. lia. }
...
Proof.
  ...
  * subst.
    ...
    exact H3. instantiate (1:=z + 1). lia. instantiate (1:=Tint). simpl.
    apply Zle_trans with (size_arguments_32 tyl (z + typesize Tlong));
      [apply Zle_trans with (z + typesize Tlong);[simpl;lia|apply size_arguments_32_above] |lia].
    simpl; apply Z.divide_1_l.
    intros (m'0 & STOREHI & SEP').
    exploit set_location; try exact INJLO.
    exact SEP'. instantiate (1:=z). lia. instantiate (1:=Tint). simpl.
    apply Zle_trans with (size_arguments_32 tyl (z + typesize Tlong));
      [apply Zle_trans with (z + typesize Tlong);[simpl;lia|apply size_arguments_32_above] |lia].
    simpl; apply Z.divide_1_l.
    ...
    { clear IHargs. destruct v'; try (simpl in H2; discriminate; fail).
      ...
      unfold fe_ofs_arg in *. replace (0 + 4 * z + 4) with (0 + 4 * (z + 1)) by lia.
    ...
    simpl. right; simpl; lia. lia. simpl; lia.
  ...
  * assert (TALIGN: typealign t = 1) by (destruct t; auto; contradiction).
    ...
    assert (OFSABOVE: 0 <= z + typesize t) by (destruct t; simpl; lia).
    replace (4 * z + AST.typesize t) with (4 * (z + typesize t)) in STOREARGS'
      by (rewrite typesize_typesize; lia).
    ...
    destruct t; auto. contradiction. lia. 
Qed.
...
Proof.
  ...
  - right. exists i. revert H. revert Hfp Lfp Lcore Lm.
      ...
      * (* Lgetstack, incoming *)
        ...
        ** (* 1. s = nil -> not tailcall possible or it is the initial function f0 *)
          ...
          { apply sep_pick2 in SEP. exploit loc_arguments_acceptable_2; eauto. intros [? ?].
            eapply get_location; eauto; try lia.
            ...
            destruct Archi.ptr64 eqn:C; inv C. destruct ARGSTYP. rewrite H3. lia. }
    ...
    + (* Lstore *)
      ...
      eapply Mem.perm_implies. eapply Mem.perm_cur_max. eapply RANGE. destruct chunk; simpl; lia.
      constructor. intros [GE RANGE].
      rewrite Ptrofs.add_unsigned. do 2 rewrite Ptrofs.unsigned_repr. auto.
      pose proof (Ptrofs.unsigned_range_2 ofs1). lia. auto.
      pose proof (Ptrofs.unsigned_range_2 ofs1). lia. 
    ...
    + (* dummy step, marshalling arguments *)
      ...
      { (** TODO: store_args -> stack contains corresponding vars *)
        exploit alloc_rule; eauto; try lia.
        ...
        repeat rewrite Zlength_correct. intros. rewrite ARGSREL in *. clear ARGSREL. lia.
        simpl in WDARGS0; InvBooleans. discriminate.
        intro ALLOCSEP. replace (4 * z) with (0 + 4 * z) in ALLOCSEP by lia.
        ...
        subst rs0. eapply store_args_rec_contains_locations; eauto. lia.
        erewrite ARGSTYP, loadframe.tyl_length_size_arguments_32, <- arg_len_rec_val; eauto. lia.
        unfold loadframe.store_args in STOREARGS. rewrite ARGSTYP. replace (4*0) with 0 by lia. auto.
      }
