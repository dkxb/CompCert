...
Proof.
  intros. destruct a; inversion H3. destruct a'; inversion H4. inversion H5; subst. apply (MemClosures_local.store_val_inject_unmapped_closed_preserved mu m1 m1' chunk inject_id b0 (Ptrofs.unsigned i0) v m2 b0 0 v' m2'); auto. apply H. apply H. apply H. replace (Ptrofs.unsigned i0 + 0) with (Ptrofs.unsigned i0). auto. lia.
Qed.
...
Lemma free_unmapped_closed_preserved:
  forall m1 m1' m2 m2' b lo hi mu (sge tge: genv),
    ...
    MemClosures_local.unmapped_closed mu m2 m2'.
Proof.
  intros. apply (MemClosures_local.free_inject_unmapped_closed_preserved mu m1 m1' inject_id b lo hi m2 b 0 lo hi m2'); auto. apply H. apply H. apply H. lia. lia.
Qed.
...
Proof.
  ...
  { inversion H4; subst.
      ...
      }
      {
        inv H_match_state. destruct i; try (right; do 4 eexists; split; [apply plus_one; constructor; remember (record_gotos_correct f pc) as Hbranch; clear HeqHbranch; rewrite H0 in Hbranch; rewrite Hbranch; rewrite <- code_preserved; rewrite H0; simpl; auto | split; [apply fp_match_union'; auto; apply fp_match_emp' | split; [constructor; auto | split; [apply fp_match_union'; auto; apply fp_match_emp' | split; auto]]]]).
        {
            ...
            simpl. auto. eresolvfp. split. eresolvfp. splitMS. }
          {
            ...
            congruence. rewrite H_branch_target. Left. simpl. lia. splitMS.
          }
        }
      }
    }
    ...
    }
    {
      ...
      {
        left. exists (measure (Core_State s f sp pc rs sigres)). split. simpl. lia. splitMS.
      }
    }
    {
      inversion H_match_state; subst.
