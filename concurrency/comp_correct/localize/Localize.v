...
Proof.
  ...
  { clear. induction N. intros. lia.
    destruct N; simpl in *. intro. rewrite Nat.add_0_r. auto.
    intros. rewrite (IHN (S s));[|lia]. simpl. repeat f_equal; lia. }
  rewrite H; auto. rewrite map_app. replace (1 + (N - 1))%nat with N by lia. simpl. auto.
Qed.
...
Proof.
  ...
  destruct (Nat.eq_dec i (length (dom x))), (Nat.eq_dec j (length (dom x))); try lia.
  (* i = B, j < B *)
  rewrite nth_error_app2 in H1; try lia. rewrite e, Nat.sub_diag in H1. simpl in H1.
  assert (j < length (dom x ++ x :: nil))%nat. apply nth_error_Some. intro. congruence.
  rewrite app_length in H2. simpl in H2. rewrite nth_error_app1 in H1; try lia.
  symmetry in H1. apply nth_error_in in H1. apply dom_correct, Plt_ne in H1. contradiction.
  (* i < B, j = B *)
  rewrite nth_error_app2 with (n:=j) in H1; try lia. rewrite e, Nat.sub_diag in H1. simpl in H1.
  ...
  assert (nth_error (dom x ++ x :: nil) j <> None). intro. rewrite H2 in H1. apply nth_error_None in H1. lia.
  apply nth_error_Some in H2. rewrite app_length in H2. simpl in H2.
  rewrite nth_error_app1 in H1; try lia.
  apply H; auto. rewrite dom_length in H0, n. lia.
Qed.
...
Proof.
  ...
  simpl. destruct peq. inv H. rewrite not_in_remove; auto. split; [lia|auto].
  inv H. apply IHD with b in H3. simpl. destruct H3. split;[lia|constructor; auto].
...
Proof.
  induction R; simpl; intros. lia.
  destruct a as [a|]; auto.
  apply nodup_remove_one with (b:=a) in H. destruct H. apply IHR in H0. unfold block in *; lia.
  apply IHR in H. lia.
Qed.
...
Proof.
  ...
  rewrite H2, RANGE in H1. repeat rewrite app_length in H1. simpl in H1. lia.
    ...
    { generalize GEREL LIFTMU valid_Src valid_Tgt rc_shared_Src rc_shared_Tgt Binj init_mu
        ...
        exfalso. destruct init_mu as [_ [_ A]]. inv J23. eapply A; unfold Bset.belongsto; eauto. }
      {
        ...
        { unfold Bset.inj_to_meminj. intros.
          destruct (j22 b) eqn:J22; inv H. split; [lia|].
