...
Proof.
  ...
  destruct (zle 4 z).
  rewrite H in H7;auto.
  right. simpl in H2. rewrite H2. lia.

  destruct (zle 3 z).
  assert(z=3). lia. subst.
  ...
  destruct (zle 2 z).
  assert(z=2). lia. subst.
  unfold inj_bytes,encode_int,rev_if_be,bytes_of_int in H7;ex_match2;simpl in H7.
  rewrite ZMap.gso in H7;[|intro R;inv R].
  rewrite ZMap.gss in H7;auto;try discriminate.

  destruct (zle 1 z).
  assert(z=1). lia. subst.
  unfold inj_bytes,encode_int,rev_if_be,bytes_of_int in H7;ex_match2;simpl in H7.
  rewrite ZMap.gso in H7;[|intro R;inv R].
  rewrite ZMap.gso in H7;[|intro R;inv R].
  rewrite ZMap.gss in H7;auto;try discriminate.

  assert(z=0). lia. subst.
  unfold inj_bytes,encode_int,rev_if_be,bytes_of_int in H7;ex_match2;simpl in H7.
  rewrite ZMap.gso in H7;[|intro R;inv R].
  rewrite ZMap.gso in H7;[|intro R;inv R].
  rewrite ZMap.gso in H7;[|intro R;inv R].
...
Proof.
  ...
  }
  {
    ...
    destruct (zle 4 z).
    rewrite H2;auto.
    right. simpl in H4. rewrite H4. lia.
...
Section MATCHSTATE.
  ...
  Proof.
    ...
    destruct check_value eqn:?.
    {
      ...
      unfold strip,GMem.perm;simpl. apply r;lia.
      intro;contradiction.
      assert(novundef0:v <> Vundef).
      inv H0. intro;subst.
      exploit no_vundef;eauto.
      unfold strip,GMem.perm;simpl;apply r;auto. lia.
      clear H H1.
      simpl in *.
      Ltac andb_split:=
    ...
    }
    {
      ...
      assert(v1 <> Vundef).
      intro;subst.
      exploit valfrag_noundef;eauto. lia.
