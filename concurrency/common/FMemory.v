...
Next Obligation.
  ...
  rewrite freelist_unique, valid_wd; unfold_fl; eauto. lia.
Qed.
Next Obligation.    
...
Next Obligation.
  ...
  assert (H1: perm m b ofs k Freeable). { apply perm_cur. apply r; lia. }
...
Proof.
  ...
  replace (p + Z.succ (Z_of_nat n1)) with ((p + 1) + Z_of_nat n1) by lia.
  auto.
Qed.
...
+ left; split. lia. unfold c'. simpl. apply setN_in.
  ...
  simpl length in H3. rewrite inj_S in H3. lia.
...
Proof.
  ...
  simpl. decEq. lia.
  simpl length. rewrite inj_S. simpl. rewrite IHbytes1. decEq. lia.
Qed.
...
Proof.
  ...
  eexact H. instantiate (1 := ofs0). rewrite app_length. rewrite inj_plus. lia.
  auto.
...
Proof. injection ALLOC; intros. subst. simpl. lia. Qed.
...
Proof.
  ...
  eapply range_perm_cur in H1. eapply H1. lia.
...
Proof.
  ...
  rewrite Z_to_nat_neg. simpl. red; intros; lia. lia.
Qed.
...
(* mem_contents *)
  ...
  rewrite setN_outside. auto.
  destruct (zlt (ofs0 + delta) ofs); auto.
  destruct (zle (ofs + Z_of_nat (length bytes2)) (ofs0 + delta)). lia.
  byContradiction. eapply H0; eauto. lia.
  eauto with mem.
Qed.
...
(* range *)
  unfold flat_inj; intros.
  destruct in_dec; inv H0. generalize (Ptrofs.unsigned_range_2 ofs); lia.
(* perm inv *)
  unfold flat_inj; intros.
