...
Proof.
  ...
  assert(z = BinInt.Z.sub j (BinInt.Z.of_nat i)). lia.
  rewrite H4.
  ...
  destruct v. apply Mem.perm_cur_max, H7. split. lia.
  rewrite size_chunk_conv. rewrite <- Heqi. lia.
  eapply H5;eauto.
Qed.
...
Proof.
  ...
  inversion H. 
  assert(BinInt.Z.add z 0 = z). lia.
  simpl. rewrite H1.
  do 2 (rewrite Mem.setN_outside;try lia).
  do 2 rewrite Maps.ZMap.gss;auto.

  inversion H.
  simpl in H.
  assert(i< length v)%nat. lia.
...
Proof.
  ...
  apply Znat.Z2Nat.inj_lt;try lia.
  assert(ofs = BinInt.Z.add i (BinInt.Z.sub ofs i)). lia.
  rewrite H2.
  eapply setN_geteq  in H1;eauto.
  rewrite Heqk. rewrite Z2Nat.id by lia. reflexivity.
Qed.
Lemma fmem_eq_store:
...
Proof.
  ...
  intro. subst. eapply valid_wd in H2; eauto. lia.
Qed.
...
Proof.
  ...
  constructor.
  {
    ...
    ex_match. assert(sz + (b0 - sz) = b0). lia.
...
Proof.
  ...
  unfold Mem.range_perm in *.
	  intros ofs' Hofs'. pose proof (r ofs' Hofs') as ?.
  destruct mempre as [_ _ R1].
  unfold effects in R1. simpl in R1. rewrite Locs.locs_union_emp in R1.
  specialize (R1 b0 ofs' ltac:(apply range_locset_belongsto2; exact Hofs') Memperm.Cur Memperm.Writable) as [R1 _].
  simpl. apply R1. exact H.
...
Proof.
  ...
  destruct Mem.perm_dec,Mem.perm_dec;auto;simpl in *;eapply H in p;eauto;try contradiction; unfold belongsto;Locs.unfolds; unfold FMemOpFP.range_locset;destruct Values.eq_block;auto;destruct zle,zlt;auto;try lia.
Qed.
...
Proof.
  ...
  subst.
  repeat destruct zle;repeat destruct zlt;auto;try lia.
Qed.
...
Proof.
  ...
  unfold FMemOpFP.range_locset. destruct Values.eq_block;auto. destruct zle,zlt;auto;lia.
...
Proof.
  ...
  constructor.
  {
    ...
    destruct zle,zlt;try lia;simpl. split;auto.

    rewrite Locs.emp_union_locs in H0. apply range_locset_belongsto in H0 as [].
    rewrite H0,Maps.PMap.gss in H1.
    destruct zle,zlt;try lia;simpl in H1. contradiction.
  }
  {
    rewrite Heqm1.
...
Proof.
  ...
  constructor.
  {
    ...
    destruct zle,zlt;try lia;simpl in H0. contradiction.
  }
  {
    unfold strip,Mem.unchecked_free,FreelistEq in *;gmem_unfolds; simpl in *.
    intros.
...
Proof.
  ...
  ex_match2.
  subst.
  destruct zle,zlt,zle,zlt;auto;try lia.
Qed.
