...
Proof.
  ...
  assert (4 | typesize t) by
      (destruct t; simpl; try replace 8 with (4 * 2) by lia; try apply Z.divide_mul_l; try apply Z.divide_refl).
  split; [apply Z.divide_add_r; auto|].
  simpl. rewrite Heqb. rewrite H0. f_equal.
  rewrite <- typesize_agree, Z_div_plus_full_l; auto. lia.
Qed.
...
Proof.
  ...
  destruct zlt; inv H1. rewrite Zlength_cons in l. lia.
Qed.
...
Proof.
  ...
  simpl in H. destruct tyl0; inv H. simpl. lia.
  ...
  rewrite Zlength_cons. destruct t; unfold Locations.typesize; try lia.
Qed.
...
Proof.
  ...
  { edestruct (Mem.valid_access_store m Mint32 sp0 ofs v).
    ...
    intros ofs' RANGE. eapply Mem.perm_store_1, H4; eauto. lia. }
  { edestruct (Mem.valid_access_store m Mfloat64 sp0 ofs v).
    unfold Mem.valid_access. simpl. split; auto. intros ? ?; apply H4; unfold Locations.typesize; try lia.
    eapply IHtyl0 in Heqo. destruct Heqo. exists x0.
    rewrite Ptrofs.add_zero_l, Ptrofs.unsigned_repr. rewrite e, H; auto. unfold Locations.typesize in *. lia.
    destruct v; auto; inv H0.
    apply Z.divide_add_r; auto. replace 8 with (4 * 2) by lia. apply Z.divide_mul_l, Z.divide_refl.
    lia. unfold Locations.typesize in *. lia.
    replace (ofs + 4 * (Locations.typesize Tfloat + z0)) with (ofs + 8 + 4 * z0) in H3 by (unfold Locations.typesize; lia).
  ...
  { destruct v; inv Heqb. inv H0.
    ...
    intros ? ?; eapply Mem.perm_store_1, H4; eauto; unfold Locations.typesize; try lia.
    eapply IHtyl0 in Heqo. destruct Heqo. exists x1. 
    repeat rewrite Ptrofs.add_zero_l, Ptrofs.unsigned_repr. rewrite e, e0; eauto.
    unfold Locations.typesize in *. lia.
    unfold Locations.typesize in *. lia.
    auto.
    apply Z.divide_add_r; auto. replace 8 with (4 * 2) by lia. apply Z.divide_mul_l, Z.divide_refl.
    lia. unfold Locations.typesize in *; lia. 
    replace (ofs + 4 * (Locations.typesize Tlong + z0)) with (ofs + 8 + 4 * z0) in H4 by (unfold Locations.typesize; lia).
    intros ofs' RANGE. eapply Mem.perm_store_1, Mem.perm_store_1, H4; eauto. lia. }
  { edestruct (Mem.valid_access_store m Mfloat32 sp0 ofs v).
    ...
    apply Z.divide_add_r; auto. apply Z.divide_refl. lia. unfold Locations.typesize in *. lia.
    replace (ofs + 4 * (Locations.typesize Tsingle + z0)) with (ofs + 4 + 4 * z0) in H4 by (unfold Locations.typesize; lia).
    intros ofs' RANGE. eapply Mem.perm_store_1, H4; eauto. lia. }
  { edestruct (Mem.valid_access_store m Many32 sp0 ofs v).
    ...
    destruct v; inv Heqb; inv H0; auto.
    apply Z.divide_add_r; auto. apply Z.divide_refl. lia. unfold Locations.typesize in *. lia.
    replace (ofs + 4 * (Locations.typesize Tint + z0)) with (ofs + 4 + 4 * z0) in H3 by (unfold Locations.typesize; lia).
    intros ofs' RANGE. eapply Mem.perm_store_1, H4; eauto. unfold Locations.typesize; lia. }
...
Proof.
  clear. induction tyl; simpl; intros; auto. rewrite IHtyl. lia.
Qed.
Lemma tyl_bytes_non_neg:
  forall tyl, tyl_bytes tyl >= 0.
Proof.
  clear. induction tyl as[|ty tyl]; simpl; [| pose proof (AST.typesize_pos ty)]; lia.
Qed.
...
Ltac red_ptr :=
  ...
  | |- context[Ptrofs.unsigned (Ptrofs.repr _)] =>
    try (rewrite Ptrofs.unsigned_repr; [| unfold Ptrofs.max_unsigned; try lia; fail])
  | H: context[Ptrofs.unsigned (Ptrofs.repr _)] |- _ =>
    try (rewrite Ptrofs.unsigned_repr in H;[| unfold Ptrofs.max_unsigned; try lia; fail])
  end.
Ltac red_triv :=
  match goal with
...
Proof.
  ...
  assert (0 < typesize a <= 8) by (destruct a; simpl; lia).
  assert (Zlength args >= 0) by (rewrite Zlength_correct; lia).
  assert (4 * (2 * Zlength args) >= 0) by lia.
  ...
  (* repeat red_triv; 
    ...
    try (eapply (Mem.unchanged_on_trans _ m m0 m');
         eapply Mem.unchanged_on_implies; simpl; eauto; intros; simpl; try intros [? ?]; red_ptr; red_triv; lia; fail).
...
Proof.
  ...
  assert (0 < typesize ty <= 8) by (destruct ty; simpl; lia).
  assert (Zlength args' >= 0) by (rewrite Zlength_correct; lia).
  assert (4 * (2 * Zlength args') >= 0) by lia.
  assert (typesize ty + 4 * (2 * Zlength args') <= 4 * (2 * (Zlength args' + 1))).
  { rewrite Z.mul_add_distr_l. lia. }
  replace (4 * 2) with 8 in * by lia.
  repeat match goal with
         | H: context[4 * (2 * ?x)] |- _ =>
           replace (4 * (2 * x)) with (8 * x) in H by lia
         | H: context[_ * (_ + _)] |- _ =>
  ...
  { unfold wd_args in *. repeat rewrite andb_true_iff in H.
    ...
    apply zlt_true. destruct zlt; [lia| inv H]. }
  ...
  destruct ty; repeat red_triv; repeat red_ptr;
    ...
    | H: store_args_rec _ _ _ _ _ = Some _ |- _ =>
      exploit store_args_rec_unchanged_on_weak; try eexact H; try lia; eauto;
      eapply IHtyl' in H; try lia; auto
    end;
    repeat split; auto; unfold load_stack, Mach.load_stack, Mem.loadv; simpl; repeat red_ptr;
      try (eapply Mem.load_unchanged_on; eauto;
           [simpl; intros; destruct eq_block; auto; lia
